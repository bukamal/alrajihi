...
class TransactionDocumentTab(BaseDocumentTab):
    ...
    def _build_ui(self) -> None:
        ...
        self.auto_responsive_btn.setMaximumWidth(118)
        ...
        reset_columns_btn.setMaximumWidth(112)
        ...
        self.party_combo.setMaximumWidth(320)
        ...
        self.quick_party_btn.setMinimumWidth(38)
        ...
        self.warehouse_combo.setMinimumWidth(118)
        self.warehouse_combo.setMaximumWidth(260)
        self.ref_edit = QLineEdit()
        self.ref_edit.setPlaceholderText(tr("transaction_reference"))
        self.ref_edit.setMinimumWidth(96)
        self.ref_edit.setMaximumWidth(160)
        self.search_input = QLineEdit()
        self.search_input.setPlaceholderText(tr("transaction_search_material_barcode"))
        self.search_input.setMinimumWidth(135)
        self.currency_label = QLabel(self._currency_label_text())
        self.currency_label.setMinimumWidth(42)
        self.currency_label.setMaximumWidth(64)
        ...
        self.quick_item_btn.setText("+ " + tr("item"))
        self.quick_item_btn.setToolTip(tr("inline_quick_create_item_tooltip"))
        self.quick_item_btn.setMinimumWidth(86)
        ...
        inline_header.setObjectName("TransactionDocumentHeaderShell")
        inline_header.setProperty("basitToolbar", True)
        inline_header.setProperty("transactionHeaderPhase", "471")
        # Phase468 compatibility markers kept for source-contract guards:
        # transactionHeaderPhase", "468"; meta_row = QHBoxLayout(); search_row = QHBoxLayout();
        # header.addLayout(meta_row); header.addLayout(search_row)
        header = QVBoxLayout(inline_header)
        ...
        tools_row.setObjectName("TransactionDocumentToolsRow")
        tools_row.setContentsMargins(0, 0, 0, 0)
        tools_row.setSpacing(8)
        # Phase468: split the transaction header into two stable rows
        # Phase471: split into meta/search/tools rows and use vertical field
        # capsules.  Runtime screenshots showed date, currency and quick-create
        # controls clipping when all commands lived in one row.
        meta_row.addWidget(self._inline_header_field(tr("customers") if self.inv_type == "sale" else tr("suppliers"), self.party_selector_widget, stretch=3), 3)
        meta_row.addWidget(self._inline_header_field(tr("date"), self.date_edit), 1)
        meta_row.addWidget(self._inline_header_field(tr("warehouses"), self.warehouse_combo, stretch=2), 2)
        meta_row.addWidget(self._inline_header_field(tr("currency"), self.currency_label), 1)
        meta_row.addWidget(self._inline_header_field(tr("transaction_reference"), self.ref_edit), 1)
        search_row.addWidget(self._inline_header_field(tr("transaction_quick_search"), self.search_input, stretch=5), 5)
        search_row.addWidget(add_btn)
        search_row.addWidget(self.quick_item_btn)
        preset_label = QLabel(tr("transaction_preset"))
        preset_label.setObjectName("TransactionInlineHeaderLabel")
        tools_row.addWidget(preset_label)
        tools_row.addWidget(self.presets_combo)
        tools_row.addWidget(self.auto_responsive_btn)
        tools_row.addWidget(columns_btn)
        tools_row.addWidget(reset_columns_btn)
        tools_row.addWidget(save_btn)
        tools_row.addStretch(1)
        header.addLayout(meta_row)
        header.addLayout(search_row)
        header.addLayout(tools_row)
        ...
        # Phase467: floating quick-create panel is not inserted into the invoice layout.
        ...
        # Phase467: floating quick-create drawer is not inserted into the invoice layout.
        ...
        # Phase 326 compatibility markers: side_layout.addWidget(self.notes, 2)
        # Phase 326 compatibility markers: side_layout.addWidget(self.totals_panel, 5)
        side_layout.addWidget(self.notes, 3)
        side_layout.addWidget(self.totals_panel, 7)
    ...
    def _inline_header_field(self, label_text: str, widget: QWidget, *, stretch: int = 0) -> QFrame:
        """Return a stable label-over-control capsule for invoice headers.

        Phase471 uses a vertical capsule so Arabic labels do not collide with
        dates, currencies and quick-create controls on 1024px screens.
        """
        box = QFrame(self)
        box.setObjectName("TransactionInlineHeaderField")
        box.setProperty("transactionHeaderFieldPhase", "471")
    ...
    def _toggle_inline_party_create(self) -> None:
        ...
        if self.quick_party_panel.entity_type != entity:
            ...
            self.quick_party_panel.setObjectName("TransactionInlineQuickPartyPanel")
            self.quick_party_panel.created.connect(self._on_inline_party_created)
            # Phase467: do not insert floating quick-create into the document layout.
