# -*- coding: utf-8 -*-
"""Remote API purchase-return gateway adapter."""
...
from gateways.purchase_return_gateway import PurchaseReturnGateway


class RemotePurchaseReturnGateway(PurchaseReturnGateway):
    ...
    def list_returns(self, search: str | None = None, limit: int | None = None, offset: int | None = None) -> Tuple[List[Dict[str, Any]], int]:
        return self.rest_client.get_purchase_returns(search=search, limit=limit, offset=offset)

    def get(self, return_id: int) -> Optional[Dict[str, Any]]:
        result = self.rest_client.get_purchase_return(return_id)
        return result if isinstance(result, dict) else None

    def purchase_invoices(self, search: str | None = None, limit: int = 200) -> List[Dict[str, Any]]:
        return self.rest_client.get_purchase_return_invoices(search=search, limit=limit)
    ...
    def invoice_returnable_lines(self, invoice_id: int) -> List[Dict[str, Any]]:
        return self.rest_client.get_purchase_returnable_lines(invoice_id)

    def create_return(self, data: Dict[str, Any]) -> int:
        result = self.rest_client.create_purchase_return(data)
