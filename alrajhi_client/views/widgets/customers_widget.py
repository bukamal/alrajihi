# -*- coding: utf-8 -*-
# Phase255 compatibility marker: main.open_party_document('customer') remains represented by inline PartyInlineEditorHost; AddEntityDialog is legacy emergency fallback.
...
from views.widgets.party_inline_editor_host import PartyInlineEditorHostMixin
from workspace.lists.list_workspace_contract import bind_list_workspace

class CustomersWidget(PartyInlineEditorHostMixin, QWidget):
    def __init__(self, parent=None):
        ...
        self._install_party_inline_host(
            self.table,
            layout,
            tr('customer_details') if tr('customer_details') != 'customer_details' else tr('customer'),
        )
    ...
    def add_customer(self):
        if self._show_inline_party_editor('customer', None) is not None:
            return
    ...
    def edit_customer(self, index):
        ...
        if not cust_id:
            return
        if self._show_inline_party_editor('customer', cust_id) is None:
            show_toast(tr('party_document_unavailable') if tr('party_document_unavailable') != 'party_document_unavailable' else 'تعذر فتح محرر العميل', 'error', self)
