...
DOCUMENT_SHELL_DESCRIPTORS: tuple[DocumentDescriptor, ...] = (
    ...
    DocumentDescriptor(
        document_type="apparel",
        shell_family=SHELL_LIST,
        title_key="apparel.workspace_title",
        i18n_scope="apparel.shell",
        settings_scope="apparel",
        gateway="product_gateway",
        api_resource="/api/items",
        network_mode=NETWORK_REMOTE_AVAILABLE,
        permissions=DocumentPermissions(view="apparel.view", create="apparel.variant", update="apparel.variant.update", print="items.print", export="apparel.export"),
        capabilities=DocumentCapabilities(save=False, print=True, export=True, delete=False, cancel=False, barcode=True, audit=True, offline_queue=False),
        currency_policy=CURRENCY_DISPLAY,
        branch_policy=BRANCH_OPTIONAL,
        audit_scope="apparel",
        workspace_route="apparel widget",
        list_route="apparel",
        document_class="views.apparel.apparel_workspace_widget.ApparelWorkspaceWidget",
        local_gateway="gateways.local.product_gateway.LocalProductGateway",
        remote_gateway="gateways.remote.product_gateway.RemoteProductGateway",
        server_blueprint="items",
        notes="Standalone Apparel UI backed by item_variants through ProductService/Gateway; no separate apparel engine.",
    ),
    DocumentDescriptor(
