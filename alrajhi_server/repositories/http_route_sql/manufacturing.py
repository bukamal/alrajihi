...
from alrajhi_server.repositories.manufacturing_repository import get_manufacturing_repository
import datetime
...
def _item_qty(db, item_id):
    row = db.query("SELECT CAST(quantity AS REAL) as qty FROM items WHERE id=?", (item_id,)).fetchone()
    return _dec(row['qty']) if row else Decimal('0')

def _item_avg_cost(db, item_id):
    row = db.query("SELECT CAST(average_cost AS TEXT) as avg_cost FROM items WHERE id=?", (item_id,)).fetchone()
    return _dec(row['avg_cost']) if row else Decimal('0')

def _default_warehouse_id(db, user_id):
    row = db.query("SELECT id FROM warehouses WHERE user_id=? AND is_default=1 AND deleted_at IS NULL AND COALESCE(is_active,1)=1 LIMIT 1", (user_id,)).fetchone()
    if row:
        return row['id']
    row = db.query("SELECT id FROM warehouses WHERE user_id=? AND deleted_at IS NULL AND COALESCE(is_active,1)=1 ORDER BY id LIMIT 1", (user_id,)).fetchone()
    if row:
        return row['id']
    return None
...
def _ensure_warehouse_balance(db, user_id, item_id, warehouse_id, unit_cost='0'):
    ...
    row = db.query("SELECT id FROM item_warehouse_balances WHERE user_id=? AND item_id=? AND warehouse_id=?", (user_id, item_id, warehouse_id)).fetchone()
    if row:
        return
...
def _record_warehouse_movement(db, user_id, item_id, warehouse_id, movement_type, quantity, unit_cost='0', reference_type='production_order', reference_id=None, notes=''):
    ...
    if qty == 0:
        return None
    wh = db.query("SELECT id FROM warehouses WHERE id=? AND user_id=? AND deleted_at IS NULL AND COALESCE(is_active,1)=1", (warehouse_id, user_id)).fetchone()
    if not wh:
...
def _update_item_quantity(db, item_id):
    ...
    db.query("UPDATE items SET quantity=? WHERE id=?", (str(qty), item_id))
...
def _recalculate_average_cost(db, item_id):
    ...
    db.query("UPDATE items SET average_cost=? WHERE id=?", (str(avg), item_id))
...
def _validate_bom_payload(db, data, user_id):
    ...
    product = db.query("SELECT id, item_type FROM items WHERE id=? AND user_id=?", (product_id, user_id)).fetchone()
    if not product:
    ...
    for line in lines:
        ...
        if unit_id:
            unit = db.query("SELECT id FROM item_units WHERE id=? AND item_id=?", (unit_id, item_id)).fetchone()
            if not unit:
...
def _get_bom_for_product(db, product_id, user_id):
    row = db.query("SELECT id FROM bom WHERE product_id=? AND user_id=?", (product_id, user_id)).fetchone()
    if row:
        bom_id = row['id']
...
def _expand_bom(db, product_id, quantity, user_id, multiplier=Decimal('1'), visited=None):
    ...
    for line in bom['lines']:
        ...
        item = db.query("SELECT item_type FROM items WHERE id=?", (item_id,)).fetchone()
...
def get_boms():
    ...
    db = get_manufacturing_repository()
    total = db.query("SELECT COUNT(*) FROM bom WHERE user_id=?", (user_id,)).fetchone()[0]
    rows = db.query("""
...
def get_bom(bom_id):
    user_id = get_jwt_identity()
    db = get_manufacturing_repository()
    bom = db.query("SELECT * FROM bom WHERE id=? AND user_id=?", (bom_id, user_id)).fetchone()
    if not bom:
...
def save_bom():
    ...
    now = datetime.datetime.now().isoformat()
    if data.get('id'):
        db.query("UPDATE bom SET product_id=?, quantity=?, updated_at=? WHERE id=? AND user_id=?",
                   (data['product_id'], str(data['quantity']), now, data['id'], user_id))
        db.query("DELETE FROM bom_lines WHERE bom_id=?", (data['id'],))
        bom_id = data['id']
    else:
        cursor = db.query("INSERT INTO bom (product_id, quantity, user_id, created_at, updated_at) VALUES (?,?,?,?,?)",
...
def delete_bom(bom_id):
    ...
    db.query("DELETE FROM bom WHERE id=? AND user_id=?", (bom_id, user_id))
...
def get_orders():
    ...
    offset = request.args.get('offset', 0, type=int)
    db = get_manufacturing_repository()
    total = db.query("SELECT COUNT(*) FROM production_orders WHERE user_id=?", (user_id,)).fetchone()[0]
    rows = db.query("""
...
def create_order():
    ...
    cur = db.query("SELECT order_number FROM production_orders ORDER BY id DESC LIMIT 1")
    last = cur.fetchone()
    if last:
...
def start_order(order_id):
    user_id = get_jwt_identity()
    db = get_manufacturing_repository()
    order = db.query("SELECT * FROM production_orders WHERE id=? AND user_id=?", (order_id, user_id)).fetchone()
    if not order:
        return jsonify({'error': 'Order not found'}), 404
    if order['status'] != 'planned':
        return jsonify({'error': f"لا يمكن بدء أمر بحالة {order['status']}"}), 400
    reservations = db.query("SELECT * FROM material_reservations WHERE order_id=?", (order_id,)).fetchall()
    if not reservations:
    ...
    db.query("UPDATE production_orders SET status='in_progress', start_date=? WHERE id=? AND user_id=?",
...
def complete_order(order_id):
    ...
    consumptions = db.query("SELECT consumed_qty, unit_cost FROM production_consumptions WHERE order_id=?", (order_id,)).fetchall()
    if not consumptions:
...
def reverse_order(order_id):
    ...
    outputs = db.query("SELECT * FROM production_outputs WHERE order_id=?", (order_id,)).fetchall()
    for o in outputs:
    ...
    consumptions = db.query("SELECT * FROM production_consumptions WHERE order_id=?", (order_id,)).fetchall()
    ...
    db.query("DELETE FROM production_consumptions WHERE order_id=?", (order_id,))
    db.query("DELETE FROM production_outputs WHERE order_id=?", (order_id,))
    db.query("DELETE FROM material_reservations WHERE order_id=?", (order_id,))
    db.query("DELETE FROM production_orders WHERE id=? AND user_id=?", (order_id, user_id))
    audit_log('POST', 'PRODUCTION_ORDER', order_id if 'order_id' in locals() else None, new_values=data if 'data' in locals() else None, details='عملية إنتاج')
    audit_log('POST', 'PRODUCTION_ORDER', order_id if 'order_id' in locals() else None, details='عملية إنتاج')
    db.commit()
    return jsonify({'status': 'ok'})
...
def order_reservations_endpoint(order_id):
    user_id = get_jwt_identity()
    db = get_manufacturing_repository()
    own = db.query('SELECT id FROM production_orders WHERE id=? AND user_id=?', (order_id, user_id)).fetchone()
    if not own:
...
def cancel_order_endpoint(order_id):
    user_id = get_jwt_identity()
    db = get_manufacturing_repository()
    row = db.query('SELECT * FROM production_orders WHERE id=? AND user_id=?', (order_id, user_id)).fetchone()
    if not row:
        return jsonify({'error': 'Order not found'}), 404
    if row['status'] == 'completed':
        return jsonify({'error': 'لا يمكن إلغاء أمر مكتمل؛ استخدم العكس'}), 400
    now = datetime.datetime.now().isoformat()
    db.query("UPDATE production_orders SET status='cancelled', end_date=? WHERE id=? AND user_id=?", (now, order_id, user_id))
    db.query('DELETE FROM material_reservations WHERE order_id=?', (order_id,))
...
def delete_consumption_endpoint(consumption_id):
    ...
    db.query('DELETE FROM production_consumptions WHERE id=?', (consumption_id,))
...
def delete_output_endpoint(output_id):
    ...
    db.query('DELETE FROM production_outputs WHERE id=?', (output_id,))
