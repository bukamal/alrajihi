...
def _phase157_has_permission(db, user_id, permission_key):
    ...
    if role and role['role'] == 'admin':
        return True
    db.execute("""INSERT OR IGNORE INTO user_roles(user_id, role_id) SELECT u.id, r.id FROM users u JOIN roles r ON lower(COALESCE(u.role,'user'))=r.name WHERE u.id=?""", (user_id,))
    return db.execute("""SELECT 1 FROM user_roles ur JOIN role_permissions rp ON rp.role_id=ur.role_id AND rp.allowed=1 JOIN roles r ON r.id=ur.role_id AND r.is_active=1 WHERE ur.user_id=? AND rp.permission_key=? LIMIT 1""", (user_id, permission_key)).fetchone() is not None
...
def accounting_journal_entries():
    ...
    rows=[dict(r) for r in db.execute('SELECT * FROM journal_entries ORDER BY id DESC LIMIT ?', (int(limit or 500),)).fetchall()]
    return jsonify({'rows': rows})
...
def accounting_balance_sheet():
    db=get_db(); as_of_date=request.args.get('as_of_date')
...
def accounting_cash_flow():
    db=get_db(); _ensure_accounting_schema_for_reports(db)
    start_date=request.args.get('start_date'); end_date=request.args.get('end_date')
    account=db.execute("SELECT id FROM accounts WHERE code='1000'").fetchone()
...
def accounting_periods():
    db=get_db(); _ensure_accounting_schema_for_reports(db)
    rows=[dict(r) for r in db.execute('SELECT * FROM accounting_periods ORDER BY end_date DESC, id DESC').fetchall()]
    return jsonify({'rows':rows})
...
def accounting_opening_balance():
    ...
    acc=db.execute('SELECT id,type FROM accounts WHERE code=?',(account_code,)).fetchone()
    equity=db.execute("SELECT id FROM accounts WHERE code='3000'").fetchone()
    if not acc or not equity:
        return jsonify({'error':'required account missing'}), 400
    next_id=db.execute('SELECT COALESCE(MAX(id),0)+1 FROM journal_entries').fetchone()[0]
    cur=db.execute("INSERT INTO journal_entries(entry_no,entry_date,source_type,source_id,description,status,created_by) VALUES (?,?, 'OPENING', NULL, ?, 'POSTED', ?)",(f'JE-{int(next_id):06d}',as_of_date,memo,get_jwt_identity()))
    ...
    for aid,d,c in lines:
        db.execute('INSERT INTO journal_lines(journal_entry_id,account_id,debit,credit,memo) VALUES (?,?,?,?,?)',(je_id,aid,str(d),str(c),memo))
...
def accounting_close_period():
    ...
    existing=db.execute('SELECT id FROM accounting_periods WHERE start_date=? AND end_date=? AND status=?',(start_date,end_date,'CLOSED')).fetchone()
    if existing:
    ...
    next_id=db.execute('SELECT COALESCE(MAX(id),0)+1 FROM journal_entries').fetchone()[0]
    cur=db.execute("INSERT INTO journal_entries(entry_no,entry_date,source_type,source_id,description,status,created_by,created_at) VALUES (?,?, 'PERIOD_CLOSE', NULL, ?, 'POSTED', ?, ?)",(f'JE-{int(next_id):06d}',end_date,f'Period closing {name}',get_jwt_identity(),now))
    ...
    for r in income.get('revenues',[]):
        ...
        if amt:
            db.execute('INSERT INTO journal_lines(journal_entry_id,account_id,debit,credit,memo) VALUES (?,?,?,?,?)',(je_id,r['account_id'],str(amt),'0','Close revenue')); total_dr += amt
    for r in income.get('expenses',[]):
        amt=Decimal(str(r.get('natural_balance') or 0))
        if amt:
            db.execute('INSERT INTO journal_lines(journal_entry_id,account_id,debit,credit,memo) VALUES (?,?,?,?,?)',(je_id,r['account_id'],'0',str(amt),'Close expense')); total_cr += amt
    re=db.execute("SELECT id FROM accounts WHERE code='3100'").fetchone(); re_id=re['id'] if hasattr(re,'keys') else re[0]
    diff=total_dr-total_cr
    if diff>0:
        db.execute('INSERT INTO journal_lines(journal_entry_id,account_id,debit,credit,memo) VALUES (?,?,?,?,?)',(je_id,re_id,'0',str(diff),'Transfer net income'))
    elif diff<0:
        db.execute('INSERT INTO journal_lines(journal_entry_id,account_id,debit,credit,memo) VALUES (?,?,?,?,?)',(je_id,re_id,str(-diff),'0','Transfer net loss'))
    curp=db.execute('INSERT INTO accounting_periods(name,start_date,end_date,status,closed_at,closed_by,closing_entry_id) VALUES (?,?,?,?,?,?,?)',(name,start_date,end_date,'CLOSED',now,get_jwt_identity(),je_id))
...
def _phase156_aging_payload(db, party, as_of_date=None):
    ...
    for row in db.execute(sql, (type_value,)).fetchall():
...
def _phase156_statement_payload(db, party, party_id, start_date=None, end_date=None):
    ...
    rows=[]
    for r in db.execute(f"SELECT date, 'INVOICE' AS source_type, id AS source_id, reference, notes, CAST({inv_debit} AS REAL) AS debit, CAST({inv_credit} AS REAL) AS credit FROM invoices WHERE {invoice_where}{date_filter}", tuple(params)).fetchall():
        rows.append(dict(r))
    for r in db.execute(f"SELECT date, 'VOUCHER' AS source_type, id AS source_id, reference, description AS notes, {v_debit_case} AS debit, {v_credit_case} AS credit FROM vouchers WHERE {voucher_where}{date_filter}", tuple(voucher_params)).fetchall():
        rows.append(dict(r))
...
def accounting_receivables_aging():
    return jsonify(_phase156_aging_payload(get_db(), 'customer', request.args.get('as_of_date')))

@reports_bp.route('/accounting/payables/aging', methods=['GET'])
@jwt_required()
def accounting_payables_aging():
    return jsonify(_phase156_aging_payload(get_db(), 'supplier', request.args.get('as_of_date')))

@reports_bp.route('/accounting/customers/<int:customer_id>/statement', methods=['GET'])
@jwt_required()
def accounting_customer_statement(customer_id):
    return jsonify(_phase156_statement_payload(get_db(), 'customer', customer_id, request.args.get('start_date'), request.args.get('end_date')))

@reports_bp.route('/accounting/suppliers/<int:supplier_id>/statement', methods=['GET'])
@jwt_required()
def accounting_supplier_statement(supplier_id):
    return jsonify(_phase156_statement_payload(get_db(), 'supplier', supplier_id, request.args.get('start_date'), request.args.get('end_date')))
