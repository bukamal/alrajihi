#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Guard for Phase 369: Warehouse-only installer and installed printing paths."""
...
MATRIX_PATH = OUT_DIR / "warehouse_installer_printing_matrix.csv"
SUMMARY_PATH = OUT_DIR / "warehouse_installer_printing_summary.json"
...
def main() -> int:
    ...
    if summary.get("issues"):
        print(f"Phase369 warehouse installer/printing guard FAILED: {summary['issues']} issues")
        for row in rows:
            if row.get("status") != "pass":
                print(f" - [{row.get('category')}] {row.get('key')}: {row.get('detail')}")
        return 1
    print(f"Phase369 warehouse installer/printing guard OK: {summary['checks']} checks / 0 issues")
