...
def main() -> int:
    ...
    checks = phase349_checks(ROOT)
    issues = phase349_issues(ROOT)
    matrix = out_dir / "transaction_entry_cash_footer_matrix.csv"
    summary = out_dir / "transaction_entry_cash_footer_summary.json"
    ...
    if issues:
        ...
        return 1
    print(f"Phase349 transaction entry/cash/footer guard passed: {len(checks)} checks / 0 issues")
    return 0
