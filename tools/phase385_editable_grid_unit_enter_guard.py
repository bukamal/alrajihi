...
OUT = ROOT / "tools" / "audit_outputs" / "editable_grid_unit_enter_matrix.csv"
SUMMARY = ROOT / "tools" / "audit_outputs" / "editable_grid_unit_enter_summary.json"
if str(CLIENT) not in sys.path:
    sys.path.insert(0, str(CLIENT))

from workspace.quality.editable_grid_unit_enter_contract import (  # noqa: E402
    editable_grid_unit_enter_matrix,
...
def main() -> int:
    ...
    if summary["issues"]:
        print(f"Phase385 editable-grid unit Enter FAILED: {summary['issues']} issues")
        for row in rows:
            if row.get("status") != "pass":
                print(f" - [{row.get('target')}] {row.get('key')}: {row.get('detail')}")
        return 1
    print(f"Phase385 editable-grid unit Enter passed: {summary['checks']} checks / 0 issues")
    return 0
